  ...
  $action = {
    ...
    switch ($ChangeType)
    {
      ...
      'Created'  { "CREATED"
          ...
          #Move-Item -Path $FullPath -Destination 

          2Pdf.exe -src $FullPath -dst "C:\Testing\FilesCopied" -options alerts:no scansf:no keepsf:yes overwrite:yes silent:yes
          Remove-Item -Path $FullPath -Force -Confirm:$false -ErrorAction Stop | Out-Null;
      }
